...
def my_istft(stft, hop_length, cuda=False):
    ...
    if cuda:
        y = y.cuda()
    ifft_window_sum = torch.zeros(batch_size, expected_signal_len,
                                  dtype=torch.float32)
    if cuda:
...
class tLN(nn.Module):
    ...
    def forward(self, inp):
        ...
        std = torch.sqrt(
            torch.sum((inp - mean) ** 2, 3, keepdim=True) / inp.shape[
                3] + self.eps)
...
class Mudv2(nn.Module):
    def __init__(self, n_fft=256, hop=125, learn_comp=False, bn_ch=32,
                 sep_ch=64, kernel=(3, 3), causal=False, layers=6, stacks=2,
                 verbose=True):
        ...
        self.kernel = kernel
        self.conv_pad = (int(np.log2((self.kernel[0] - 1) / 2)),
                         int(np.log2((self.kernel[1] - 1) / 2)))

        self.enc_LN = tLN(2)
        self.BN = nn.Conv2d(2, self.BN_channel, (1, 1))

        self.compression = nn.Parameter(torch.ones(1, ) * -0.8475,
                                        requires_grad=learn_comp)  # sigmoid(-0.8475) = 0.3
        if verbose:
        ...
        for s in range(self.stack):
            for i in range(self.layer):
                self.conv.append(
                    DepthConv2d(self.BN_channel, self.conv_channel,
                                self.kernel, dilation=(2 ** i, 2 ** i),
                                causal=causal,
                                padding=(2 ** (i + self.conv_pad[0]),
                                         2 ** (i + self.conv_pad[1]))))
                if s == 0 and i == 0:
        ...
        if verbose:
            print('Receptive field TIME: {:1d} samples.'.format(
                self.receptive_field_time))
            print('Receptive field FREQ: {:1d} samples.'.format(
                self.receptive_field_freq))
        ...
        self.reshape1 = nn.Sequential(nn.Conv2d(self.BN_channel, 1, (1, 1)),
                                      nn.Tanh())

        self.reshape_noise = nn.Sequential(
            nn.Conv2d(self.BN_channel, 1, (1, 1)), nn.Sigmoid())
        self.reshape_speech = nn.Sequential(
            nn.Conv2d(self.BN_channel, 1, (1, 1)), nn.Sigmoid())
    ...
    def forward(self, x):
        ...
        all_mics_stft = torch.stft(all_mics, self.FFT, self.HOP,
                                   window=win)  # (BxM, F, L, 2)

        all_mics_stft = all_mics_stft.contiguous().view(batch_size, n_mic,
                                                        all_mics_stft.shape[1],
                                                        all_mics_stft.shape[2],
                                                        2)  # (B, M, F, L, 2)
        all_mics_stft = all_mics_stft.permute(0, 1, 4, 2, 3)
        all_mics_stft = all_mics_stft.contiguous().view(-1,
                                                        all_mics_stft.shape[2],
        ...
        to_sum = skip_connection1.contiguous().view(batch_size, n_mic,
                                                    skip_connection1.shape[1],
                                                    skip_connection1.shape[2],
                                                    skip_connection1.shape[
                                                        3])  # (B, M, BN, F, L)
        ...
        mask_noise = self.reshape_noise(skip_connection).permute(0, 2, 1,
                                                                 3).unsqueeze(
            2)  # B, F, 1, 1, T
        mask_speech = self.reshape_speech(skip_connection).permute(0, 2, 1,
                                                                   3).unsqueeze(
            2)  # B, F, 1, 1, T
        ...
        observation = all_mics_stft.contiguous().view(batch_size, n_mic, 2,
                                                      ...
                                                      all_mics_stft.shape[
                                                          3]).permute(0, 3, 2,
                                                                      ...
                                                                      4)  # (B, F, 2, M, L)
        # calculate psds
        psd_noise = complex_psd(observation, mask_noise,
                                normalize=False)  # (B, F, 2, M, M)
        psd_speech = complex_psd(observation, mask_speech,
                                 condition=True)  # (B, F, 2, M, M)

        psd_noise_cond = condition_covariance(psd_noise, 1e-6)
        psd_noise_norm = psd_noise_cond / trace(psd_noise_cond, dim1=-1,
                                                dim2=-2, keepdim=True)[:, :,
                                          0].unsqueeze(2)
        ...
        a = torch.cat(
            [torch.cat([re_a, -im_a], -1), torch.cat([im_a, re_a], -1)], -2)
        b = torch.cat(
            [torch.cat([re_b, -im_b], -1), torch.cat([im_b, re_b], -1)], -2)
        ...
        real = einsum(a, c, '...ab,...bac->...ac') - einsum(b, d,
                                                            '...ab,...bac->...ac')
        imag = einsum(a, d, '...ab,...bac->...ac') + einsum(b, c,
                                                            '...ab,...bac->...ac')  # (B, F, L)

        filtered = torch.cat([real.unsqueeze(-1), imag.unsqueeze(-1)],
                             -1)  # B, F, T, 2
        output = my_istft(filtered, hop_length=self.HOP,
                          cuda=filtered.is_cuda)  # B, T
